...
int main() {
	...
	std::stack<std::string> st_c;
	st_c.push("ccc");

	std::cout << "=== TEST CONST STACK ===" << std::endl;
	std::cout << "=== test stack() && size() ===" << std::endl;
	const std::stack<std::string> st_b;
	...
	std::stack<std::string, std::list<std::string> > yy(zz);
	...
	std::stack<std::string, std::vector<std::string> > ww(xx);
